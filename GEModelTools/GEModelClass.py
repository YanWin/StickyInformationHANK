...
class GEModelClass:
    ...
    def _find_i_and_w_path(self):
        ...
        if len(self.grids_hh) == 1:
            ...
            grid1 = getattr(par, f'{self.grids_hh[0]}_grid')
            grid2 = getattr(par, f'{self.grids_hh[1]}_grid')
            find_i_and_w_2d_1d_path(par.T, path_pol1, grid1, grid1, grid2, path.pol_indices[:, 0],
                                    path.pol_weights[:, 0])
            find_i_and_w_2d_1d_path(par.T, path_pol2, grid2, grid1, grid2, path.pol_indices[:, 1],
                                    path.pol_weights[:, 1])
    ...
    def _calc_jac_hh_fakenews(self, inputs_hh_all=None, dx=1e-4, do_print=False):
        ...
        with jit(self) as model:
            ...
            simulate_hh_forwards_endo(ss.D, one_step_ss['pol_indices'], one_step_ss['pol_weights'],
                                      one_step_ss['Dbeg_plus'])
            ...
            for inputname in inputs_hh_all:
                ...
                for s in range(par.T):
                    ...
                    simulate_hh_forwards_endo(ss.D, stepvars_hh['pol_indices'], stepvars_hh['pol_weights'],
                                              stepvars_hh['Dbeg_plus'])
        ...
        for inputname in self.inputs_hh_all:
            for outputname in self.outputs_hh:
                ...
                if inputname not in inputs_hh_all:
                    ...
                    F[1:, :] = curly_E[outputname].reshape((par.T - 1, -1)) @ curly_Dbeg1[inputname].reshape(
                        (par.T, -1)).T
    ...
    def _compute_jac(self, inputs=None, dx=1e-4, do_print=False, parallel=True):
        ...
        if do_print:
            if do_unknowns:
                print(
                    f'full Jacobian to unknowns computed in {elapsed(t0)} [in evaluate_path(): {elapsed(0, evaluate_t)}]')
            else:
                print(
                    f'full Jacobian to shocks computed in {elapsed(t0)} [in evaluate_path(): {elapsed(0, evaluate_t)}]')
    ...
    def prepare_simulate(self, skip_hh=False, only_pols_hh=True, reuse_G_U=False, do_print=True):
        ...
        for i_shock, shockname in enumerate(self.shocks):
            ...
            for varname in self.varlist:
                ...
                for inputname in self.unknowns:
                    self.IRF[(varname, shockname)][:] += self.jac[(varname, inputname)] @ self.IRF[
                        (inputname, shockname)]
    ...
    def simulate(self, do_prepare=True, skip_hh=False, only_pols_hh=True, reuse_G_U=False, do_print=False):
        ...
        if do_prepare: self.prepare_simulate(skip_hh=skip_hh, only_pols_hh=only_pols_hh, reuse_G_U=reuse_G_U,
                                             do_print=do_print)
