...
class GEModelClass:
    ...
    def _calc_jac_hh_direct(self,jac_hh,inputname,dx=1e-4,do_print=False,s_list=None):
        ...
        # solve hh problem for given path and give new policies, indices and weights
            # shock happens in the last period
            # inputs are taken from path.input[0, t] and outputs are set into path.output[t]
        self.solve_hh_path()
        ...
        for s in s_list:
            ...
            # i. before shock only time to shock matters
                # set the first s+1 paths.x's to the last s+1
                # if s=[0,50] -> shock will be in path[0,50]
                # and accordingly weigths/indices/policies change in path[0,50]
    ...
    def _calc_jac_hh_fakenews(self,inputs_hh_all=None,dx=1e-4,do_print=False):
        ...
        with jit(self) as model:
            ...
            for inputname in inputs_hh_all:
                ...
                for s in range(par.T):
                    ...
                    # ii. curly_Y0
                        # derivatives of the outputs at time 0 to a shock at time s
                        # solved by one complete backward iteration of the household problem and then multiplying with
                        # the steady state distribution
                        # s.t. for iteration s, the shock is s periods in the future
                    for outputname in self.outputs_hh:
                    ...
                    # ii. curly_Dbeg1
                        # derivatives of the distribution at time 1 to a shock at time s
                        # curly_Dbeg1[s] gives the distribution if shock is s periods away
                        # with backwards iteration of household problem, we get the policy function if the shock is s
                        # periods away.
                        # Then, the indices and weights given this policy function are found and one forward iteration
                        # using these indices and ss.D gives the distribution in t=1
        ...
        # c. step 3: expectation factors
            # simply derived by iterating the steady state policy function and using the transposed simulations
        t0 = time.time()
        ...
        for inputname in self.inputs_hh_all:
            for outputname in self.outputs_hh:
                ...
                if inputname not in inputs_hh_all:
                    ...
                    # # alternative, but same results
                    # F = np.empty((par.T,par.T))
                    # F[0, :] = curly_Y0[(Outputname_hh, inputname)]
                    # for s in range(par.T):
                    #     for t in range(0, par.T - 1):
                    #         F[t + 1, s] = np.sum(curly_E[outputname][t] * curly_Dbeg1[inputname][s])
