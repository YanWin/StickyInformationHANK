...
def prepare_hh_ss(model):
    ...
    for i_fix in range(par.Nfix):
        ...
        for i_a in range(par.Na):
            ss.vbeg_l_a[i_fix, :, :, i_a] = ss.z_trans[i_fix] @ v_a[i_fix, :, :, i_a]
...
def objective_ss(x, model, do_print=False):
    ...
    model._compute_jac_hh(do_print=False, inputs_hh_all=['eg_transfer'], s_list=[0], t_list=[0,1,2,3])
...
def find_ss(model, do_print=False):
    ...
    objective_ss([par.beta_mean, par.sigma_e], model, do_print=do_print)
    ...
    if do_print:
        ...
        print(f'Discrepancy in MPC = {ss.clearing_MPC:12.8f}')